...
static dvdnav_status_t dvdnav_open_common(dvdnav_t** dest,
  ...
  if(!vm_reset(this->vm, path, priv, stream_cb, dir_cb)) {
    goto fail;
  }
...
dvdnav_status_t dvdnav_open_files(dvdnav_t **dest,
                             const dvdnav_logger_cb *logcb,
                             const char *path, void *priv, dvdnav_stream_cb *stream_cb, dvd_reader_dir_cb *dir_cb) {
  return dvdnav_open_common(dest, priv, logcb, path, stream_cb, dir_cb);
}
